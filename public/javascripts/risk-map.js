...
(function () {
  ...
  function updatePrediction(lat, lng) {
    ...
    fetch('/api/disaster-predict', {
      ...
      .then(function (data) {
        var disasterName = data && data.disaster ? data.disaster : 'Not classified';
        ...
        confEl.textContent = confidence != null ? Math.round(confidence) + '%' : '–';
        riskEl.textContent = riskLabel;

        // Show a live alert banner when conditions are harmful
        if (alertBanner && alertText && confidence != null && confidence >= 60) {
          alertText.textContent = riskLabel + ' — ' + disasterName + ' near your current location.';
      ...
      .catch(function () {
        ...
        riskEl.textContent = 'Live model not reachable — showing static map only.';
        if (alertBanner) {
          alertBanner.style.display = 'none';
        }
      });
  ...
  function placeUserMarker(lat, lng, accuracyMeters, label) {
    ...
    userMarker = L.circleMarker([lat, lng], {
      ...
      fillColor: '#22c55e',
      fillOpacity: 0.9,
    }).addTo(userLayer).bindPopup(label || 'Your live location');
  ...
  function initMap() {
    ...
    L.circle([19.8, 92.5], {
      ...
      fillOpacity: 0.35,
      radius: 180000,
    }).addTo(criticalLayer).bindPopup('Critical impact zone');
    ...
    L.circle([20.8, 92.2], {
      ...
      fillOpacity: 0.2,
      radius: 260000,
    }).addTo(severeLayer).bindPopup('Severe warning zone');
    ...
    L.circle([21.5, 93.2], {
      ...
      fillOpacity: 0.18,
      radius: 320000,
    }).addTo(watchLayer).bindPopup('Watch zone');
    ...
    cities.forEach(function (c) {
      L.circleMarker(c.coords, {
        ...
        fillColor: '#00b4ff',
        fillOpacity: 0.8,
      }).addTo(cityLayer).bindPopup(c.name + ' — ' + c.risk + ' risk');
    });
    ...
    if ('geolocation' in navigator) {
      ...
      navigator.geolocation.watchPosition(
        ...
        function (err) {
          ...
          if (riskEl) {
            if (err && err.code === 1) {
              riskEl.textContent = 'Location access blocked — enable location for VORTEX AI in your browser settings.';
            } else {
              riskEl.textContent = 'Unable to read live location — check GPS / browser settings.';
            }
          }
          if (nameEl) {
      ...
      if (riskEl) {
        riskEl.textContent = 'Live location not available — open VORTEX AI on https:// or http://localhost and enable location, or use Manual location mode.';
      }
      if (nameEl) {
