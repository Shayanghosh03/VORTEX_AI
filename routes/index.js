...
router.get('/', function (req, res, next) {
  res.render('index', {
    title: 'VORTEX AI | Disaster Damage Analysis',
...
router.post('/damage-model', requireAuth, function (req, res, next) {
  const { hazardType, windSpeed, rainfall, populationDensity, infrastructureScore, latitude, longitude } = req.body || {};
  ...
  const infra = Math.min(100, Math.max(0, parseFloat(infrastructureScore) || 0));
  ...
  const modelReq = http.request(options, function (modelRes) {
    ...
    modelRes.on('end', function () {
      ...
      try {
        ...
        result.mlConfidence = typeof parsed.confidence === 'number' ? parsed.confidence : null;
...
router.get('/forecasts/download', requireAuth, function (req, res, next) {
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="vortex_72h_forecast.csv"');
  res.send(rows);
});

router.get('/response', requireAuth, function (req, res, next) {
  res.render('response', {
    title: 'VORTEX AI | Response',
    activePage: 'response',
  });
});
...
router.get('/response-console', requireAuth, function (req, res, next) {
  res.render('response-console', {
    title: 'VORTEX AI | Response Console',
    activePage: 'response',
  });
});
...
router.get('/response/tasks/export', requireAuth, function (req, res, next) {
  ...
  res.setHeader('Content-Type', 'text/csv');
  res.setHeader('Content-Disposition', 'inline; filename="vortex_tasking_log.csv"');
  res.send(rows);
});
...
router.get('/reports', requireAuth, function (req, res, next) {
  res.render('reports', {
    title: 'VORTEX AI | Reports',
    activePage: 'reports',
  });
});

router.post('/reports/new', requireAuth, function (req, res, next) {
  const { title, incident, region, window, format, audience, notes } = req.body || {};
  ...
  res.setHeader('Content-Disposition', 'attachment; filename="vortex_report_payload.json"');
  res.send(JSON.stringify(payload, null, 2));
});

router.get('/reports/exports', requireAuth, function (req, res, next) {
  res.render('reports-exports', {
    title: 'VORTEX AI | All Exports',
    activePage: 'reports',
  });
});
